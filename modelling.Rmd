...
The response, the two-party preferred vote in favour of the Liberal party (denoted $Y$), lies in the interval $(0,100)$, where $Y = 70$ would represent a 70% preference for Liberal, 30% for Labor. Since the observed values of $Y$ are never very close to 0 or 100 (minimum $24.05 \%$ and maximum $74.90 \%$), there is no need to impose the constraint of $Y \in [0,100]$. Electorates represent aggregate spatial units, and a Moran's I test of the two-party preferred vote finds significant spatial autocorrelation for each election ($p \le 7\cdot10^{-15}$), so the spatial structure of the data must be taken into account. The spatial error model [@Anselin99] is chosen because it allows for spatial correlation in the errors. This can be thought of as capturing the political climate, where political pressures (unobserved characteristics influencing voter behaviour) of neighbours influence an electorate's voting behaviour. Spatial weights are calculated by allowing each electorate to be equally correlated with any electorate that shares any part of its boundary. The spatial error model is specified as follows:

Let $\rho$ be the parameter governing the extent of spatial correlation, $\boldsymbol v$ be a spherical error term, ${\bf W}$ be a matrix of spatial weights (containing information about the neighbouring regions), $\bf X$ be a matrix of socio-demographic covariates, $\boldsymbol \beta$ be a vector of regression coefficients and ${\boldsymbol u}$ be a vector of spatially correlated errors.

$${\bf y} = {\bf X} {\boldsymbol \beta} + {\boldsymbol u}$$

with

$${\boldsymbol u} = \rho {\boldsymbol W} {\boldsymbol u} + {\boldsymbol v}$$
where 
...
Estimation is done via maximum likelihood (using the `spdep` `R` package [@spdep]).
