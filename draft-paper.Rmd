...
```{r hume2, dependson='hume1'}
hume %>%
  select(DivisionNm, AusCitizen, Pst) %>%
  kable(longtable=FALSE, booktabs=TRUE, digits=2,
        format='latex',escape=FALSE,
        col.names=c(
          "Source zone (2016)",
          "AusCitizen (\\%): $D_s$",
...
A weighted average of *AusCitizen*, with the allocated population from each source zone as weights, yields $\hat{C}_{\text{Hume},2016} = `r sprintf("%4.2f",wave2016)`\%$. Repeating this process using the 2011 Census yields $\hat{C}_{\text{Hume},2011} = `r sprintf("%4.2f",wave2011)`\%$. Finally, linear interpolation between 2011 and 2016 yields the 2013 estimate:
$$
