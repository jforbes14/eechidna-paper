...
The final predictor set contains \(p=32\) variables\footnote{Mean of all variables aside from Judaism, Indigenous, Islam and Buddhism, where it assumes the mean of the log value.} which are listed in Table \ref{tab:printmodels}.
...
\caption{Partial residual plots by election year for a selection of predictors. Linear model with 95\% confidence bands overlaid. Most predictors have a positive relationship: the larger the value the more likely the electorate preferences the Coalition. The relationship is relatively robust over time, with the exception of \texttt{OtherLanguageHome} and \texttt{Pop\_00\_19}.}\label{fig:partresplot1}
\end{figure}
