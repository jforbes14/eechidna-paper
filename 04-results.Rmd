...
# Grid visreg
grid_visreg <- function(varname, xvar_title = "", xlimits = NULL, ylimits = NULL, plot = FALSE, table = TRUE, years = FALSE) {
  if (plot == TRUE) {
    ...
    if (table == TRUE) {
      plots <- grid.arrange(p01, p04, p07, p10, p13, p16, nrow = 1, 
        bottom = grid.text(xvar_title, gp = gpar(fontface = "bold", cex = 0.6)))
    } else {
      plots <- grid.arrange(p01, p04, p07, p10, p13, p16, nrow = 1, 
...
ggplot(data = all_bands_df %>% filter(varname %in% vars_to_plot[13])) +
  geom_ribbon(aes(x = variable, ymin = lower95, ymax = upper95), fill = "grey80") + 
  geom_point(aes(x = variable, y = part_res), data = all_points_df %>% filter(varname %in% vars_to_plot[13]), size = 0.75, col = "grey50") + 
  geom_line(aes(x = variable, y = fitted), col = "blue", size = 1) +
  theme_bw() + 
  labs(x = varname, y = "Response") + 
  facet_wrap(varname ~ year, ncol = 6, scales = "free") +
  expand_limits(x = c(-2, 2))
  expand_limits(x = my_x_ranges[25:26], y = my_y_ranges[25:26])
