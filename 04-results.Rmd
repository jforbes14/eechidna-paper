...
#### *Move the individual plots in each section to a global grid here*
...
ggplot(data = all_bands_df %>% filter(varname %in% vars_to_plot[1:6])) +
  geom_ribbon(aes(x = variable, ymin = lower95, ymax = upper95), fill = "grey80") + 
  geom_point(aes(x = variable, y = part_res), data = all_points_df %>% filter(varname %in% vars_to_plot[1:6]), size = 0.5, shape = 1, alpha = 0.5) + 
  geom_line(aes(x = variable, y = fitted), col = "blue", size = 1) +
  theme_bw()+ 
  labs(x = "", y = "Two-party preferred vote (%)") + 
  facet_grid(varname ~ year, scales = "free_x") + theme(strip.text.x = element_blank())


my_x_ranges <- c(range(model_df$Judaism), range(model_df$Born_Asia), range(model_df$Born_SE_Europe), range(model_df$OtherLanguageHome), range(model_df$Education), range(model_df$MedianAge), range(model_df$DeFacto), range(model_df$DiffAddress), range(model_df$ManagerAdminClericalSales), range(model_df$Transformative), range(model_df$Extractive), range(model_df$Unemployment), range(model_df$Incomes))
...
ggplot(data = all_bands_df %>% filter(varname %in% vars_to_plot[1])) +
  geom_ribbon(aes(x = variable, ymin = lower95, ymax = upper95), fill = "grey80") + 
  geom_point(aes(x = variable, y = part_res), data = all_points_df %>% filter(varname %in% vars_to_plot[1]), size = 0.5, shape = 1, alpha = 0.5) + 
  geom_line(aes(x = variable, y = fitted), col = "blue", size = 1) +
  theme_bw()+ 
  labs(x = "", y = "") + 
  facet_grid(varname ~ year, scales = "free") + theme(strip.text.x = element_blank()) +
  expand_limits(x = my_x_ranges[1:2], y = my_y_ranges[1:2])
...
p_Judaism <- grid_visreg("Judaism", xvar_title = "Judaism", xlimits = c(-1.5, 3.8), ylimits = c(40, 70), plot = T, years = T)
p_Born_Asia <- grid_visreg("Born_Asia", xvar_title = "Born_Asia", xlimits = c(-0.9, 5), ylimits = c(20, 70), plot = T)
p_Born_SE_Europe <- grid_visreg("Born_SE_Europe", xvar_title = "Born_SE_Europe", xlimits = c(-0.9, 5), ylimits = c(20, 70), plot = T)
p_OtherLanguageHome <- grid_visreg("OtherLanguageHome", xvar_title = "OtherLanguageHome", xlimits = c(-1.1, 3.5), ylimits = c(5, 80), plot = T)
p_Education <- grid_visreg("Education", xvar_title = "Education", xlimits = c(-2.5, 2.7), ylimits = c(31, 70), plot = T)

grid.arrange(p_Judaism, p_Born_Asia, ncol = 1, left = grid.text("Two-party preferred vote (%)", gp = gpar(cex = 0.8), rot = 90))
