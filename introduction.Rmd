...
Previous work on modelling voting outcomes with aggregate socio-demographics find that relatively good models can be fit. @Forrest01 does this using multiple regression of the Liberal and Labor primary vote for polling booths in the Farrer electorate in 1998. @Stimson06, @Stimson09 and @Stimson12 conduct principal component analysis of polling booths in the 2001, 2004 and 2007 elections respectively, also finding that socio-demographic characteristics of polling booths are linked to their two-party preferred vote. @Stimson09 model the polling booth swing vote (change in the two-party preferred vote) in the 2007 election, but had little success. Instead of analyzing a single election in isolation, this paper employs a consistent model framework across six elections so that any temporal changes are observed, where each federal elections is modelled with a cross-sectional data set. Each cross-section consists of the two-party preferred vote (response variable), and socio-demographic variables (explanatory variables) that characterise each electorate. This process involves standardizing variables, using principal components to group variables into "factors" and finally fitting a spatial error model. These steps, and the rational behind a spatial model, are discussed in @CHAPTER4. Inference is then conducted on the models and insights are discussed in @CHAPTER5.
